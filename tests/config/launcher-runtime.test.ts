...
describe("launcher process identity", () => {
  const identity = { kind: "electron", label: "TapeBox", executable: "TapeBox" };

  it("owns only this repository's TapeBox runtime", () => {
    expect(ownsProcess({
      pid: 10,
      parentPid: 1,
    ...
    expect(ownsProcess({
      pid: 11,
      parentPid: 1,
      executablePath: `${REPO_ROOT}/dist/mac-arm64/TapeBox.app/Contents/MacOS/TapeBox`,
      commandLine: "",
    }, identity)).toBe(true);
    expect(ownsProcess({
      pid: 12,
      parentPid: 1,
      executablePath: `${REPO_ROOT}\\dist\\win-unpacked\\TapeBox.exe`,
      commandLine: "",
    }, identity)).toBe(true);
    expect(ownsProcess({
      pid: 13,
