    ...
    return (
      <main className="flex h-screen items-center justify-center bg-canvas p-8 text-fg-strong" role="alert">
        <div className="w-full max-w-xl space-y-3">
          ...
          <p className="text-sm text-fg">Reload the window to recover. Your library files are unchanged.</p>
          <button type="button" onClick={() => window.location.reload()} className="rounded border border-line-strong bg-raised px-3 py-1.5 text-sm hover:bg-raised-hover">Reload window</button>
        </div>
      </main>
    )
  }
