/**
 * Pinned OpenPGP public key for evermeet.cx, which signs every static macOS
 * ffmpeg build it publishes. evermeet exposes no SHA-256 — only a detached
 * signature beside each archive — so this key is the trust anchor that lets us
 * verify a downloaded ffmpeg before making it executable (see integrity.ts).
 *
 * User ID: "static FFmpeg binaries (signing key) <ffmpeg@evermeet.cx>".
 * Source : https://evermeet.cx/ffmpeg/0x1A660874.asc (linked from the downloads
 *          page; the same fingerprint is searchable on keys.openpgp.org).
 *
 * EVERMEET_PRIMARY_FINGERPRINT is asserted against the parsed key at verify time,
 * so an accidentally corrupted or swapped key block fails loudly rather than
 * silently trusting the wrong identity.
 */
export const EVERMEET_PRIMARY_FINGERPRINT = '20f6ea3e0cfd6b4c53447a73476c4b611a660874'

export const EVERMEET_PUBLIC_KEY_ARMORED = `-----BEGIN PGP PUBLIC KEY BLOCK-----


t3FlBVsKtBiOa6FvDptN
=Q36a
-----END PGP PUBLIC KEY BLOCK-----`
