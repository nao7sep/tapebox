...
export interface BackupIndexEntry {
  ...
  archivedAt: string
  /** The file's full entry path within the zip, e.g. `catalog.json`. */
  archivePath: string
...
/**
 * The whole ledger — a JSON object wrapping the `entries` array (never a bare array), so future
...
export interface BackupReport {
  ...
  /** Files skipped (unreadable, missing, or a case-insensitive collision), each with a reason. */
  skips: BackupSkip[]
