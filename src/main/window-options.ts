 ...
 * Electron at module load). Electron owns durable bounds persistence for this
 * stable window identity; the opening size remains the fallback when no usable
 * state exists. The minimums are DERIVED in @shared/layout from the pane
 * minimums plus fixed chrome, so the OS floor can never let a pane be squeezed
