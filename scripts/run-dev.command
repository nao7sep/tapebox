...
RENDERER_URL="http://127.0.0.1:27143"
...
pause_on_failure() {
  ...
  if [[ "$status" -ne 0 && ( "$status" -lt 128 || "$status" -gt 143 ) ]]; then
    echo
    echo "tapebox run-dev failed with exit code $status."
...
cleanup() {
  ...
  if node "$RUNTIME_HELPER" is-owner "$RUNTIME_TOKEN" >/dev/null 2>&1; then
    node "$RUNTIME_HELPER" stop-if-owner "$RUNTIME_TOKEN" electron "TapeBox" "TapeBox" >/dev/null 2>&1 || true
  else
    status=0
  fi
...
log_step "Replacing any existing TapeBox runtime"
node "$RUNTIME_HELPER" claim "$RUNTIME_TOKEN"
node "$RUNTIME_HELPER" stop electron "TapeBox" "TapeBox"
node "$RUNTIME_HELPER" check-endpoint 127.0.0.1 27143
...
log_step "TapeBox is ready at $RENDERER_URL"
wait "$DEV_PID"
